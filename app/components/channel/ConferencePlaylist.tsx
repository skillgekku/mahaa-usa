...
import type { YouTubeVideo, ChannelConfig } from '@/app/lib/types';
...
export default function ConferencePlaylist({ conference, onBack, onPlayVideo }: ConferencePlaylistProps) {
  ...
  return (
    <div className={`min-h-screen ${theme.body}`}>
      ...
      <div className={`bg-gradient-to-r ${conference.bgGradient} text-white py-4 sm:py-6 px-3 sm:px-6`}>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
            <div className="flex items-center space-x-3 sm:space-x-4">
              ...
              <div className="min-w-0">
                <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold truncate">{conference.name}</h1>
                <p className="text-gray-100 flex items-center space-x-1 sm:space-x-2 text-sm sm:text-base">
                  ...
                  <span className="hidden sm:inline">{conference.description} - YouTube Playlist</span>
      ...
      <div className="container mx-auto px-3 sm:px-6 py-4 sm:py-8">
        ...
        {currentVideo && viewMode === 'schedule' && (
          <div className={`bg-gradient-to-r ${conference.bgGradient} rounded-xl p-4 sm:p-6 mb-6 sm:mb-8 text-white overflow-hidden relative`}>
            ...
            />
            <div className={`absolute inset-0 bg-gradient-to-r ${conference.bgGradient.replace('from-', 'from-').replace('to-', 'to-')} opacity-90`} />
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
                <div className="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
                  ...
                  <div className="text-center sm:text-left">
                    ...
                    <p className="text-white/80 mt-2 text-sm line-clamp-2 sm:line-clamp-none">{currentVideo.description}</p>
                  </div>
                </div>
                <button
              ...
              {getScheduledVideos().map((video, index) => (
                <div
                  ...
                  className={`${theme.card} rounded-xl p-4 sm:p-6 border transition-all duration-300 hover:shadow-lg cursor-pointer touch-target ${
                    video.isCurrentlyPlaying ? `border-opacity-100 shadow-lg` : 'hover:border-gray-300'
                  }`}
                  style={{
                    borderColor: video.isCurrentlyPlaying ? conference.color === 'blue' ? '#3b82f6' : 
            ...
            <>
              <div className="col-span-full mb-4 sm:mb-6">
                <h2 className={`text-xl sm:text-2xl font-bold ${theme.title} flex items-center space-x-2`}>
                  ...
                  {isShuffled && (
                    <span className={`text-base sm:text-lg`} style={{ color: conference.color === 'blue' ? '#3b82f6' : 
                           conference.color === 'green' ? '#10b981' :
                           conference.color === 'red' ? '#ef4444' :
                           conference.color === 'purple' ? '#8b5cf6' :
                           conference.color === 'yellow' ? '#f59e0b' : '#6b7280' }}>
                      • Shuffled
                    </span>
                  )}
                </h2>
