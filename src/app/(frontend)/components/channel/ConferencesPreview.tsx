...
const ConferenceCarousel: React.FC<ConferenceCarouselProps> = ({
  ...
  // Auto-play effect
  useEffect(() => {
    if (!isPreviewAutoPlaying || isPreviewHovered || conferences.length === 0) return

    const interval = setInterval(() => {
      setPreviewIndex((prev) => (prev >= conferences.length - itemsPerView ? 0 : prev + 1))
    }, 5000)
  ...
  const prevPreviewSlide = (): void => {
    setIsPreviewAutoPlaying(false)
    setPreviewIndex((prev) => (prev === 0 ? conferences.length - itemsPerView : prev - 1))
    setTimeout(() => setIsPreviewAutoPlaying(true), 10000)
  }
    ...
    return (
      <div className="mt-8">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-white text-xl md:text-2xl font-bold">Indian Organizations</h3>
        </div>
          ...
          {conferences.map((conference, index) => (
            <button
              ...
              style={{
                ...
                borderColor: getBorderColor(conference.color, index === currentIndex),
              }}
            >
              <div className="text-center">
                ...
                <h4 className={`${theme.title} font-semibold text-sm md:text-md mb-1 line-clamp-2`}>
                  {conference.name}
                </h4>
